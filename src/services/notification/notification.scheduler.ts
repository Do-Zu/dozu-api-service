...
export class NotificationScheduler {
      ...
      try {
        ...
        for (const user of users) {
          try {
            ...
            logger.error(`Failed to send daily reminder to user ${user.username} (${user.userId}):`, error);
            failed++;
          }
        }
      ...
      try {
        ...
        for (const user of users) {
          try {
            ...
            logger.error(`Failed to send weekly report to user ${user.username} (${user.userId}):`, error);
            failed++;
          }
        }
      ...
      try {
        ...
        for (const user of inactiveUsers) {
          try {
            ...
            if (success) {
              sent++;
              logger.debug(`Re-engagement notification sent to user ${user.username} (${user.userId})`);
            } else {
              failed++;
            }
          } catch (error) {
            logger.error(`Failed to send re-engagement notification to user ${user.username} (${user.userId}):`, error);
            failed++;
          }
        }
   ...
   */
  static async triggerAchievementNotification(userId: number, achievement: {
    title: string;
    description: string;
    icon: string;
  }): Promise<boolean> {
    ...
    try {
      ...
      for (const user of users) {
        try {
          ...
          logger.error(`Failed to send system announcement to user ${user.username} (${user.userId}):`, error);
          failed++;
        }
      }
