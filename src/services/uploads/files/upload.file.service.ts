...
export class UploadFileService {
  ...
  private validateFile(file: MulterFile, config?: FileUploadConfig): FileValidationResult {
    ...
    if (
      ...
      !currentConfig.allowedExtensions.includes(fileExtension)
    ) {
      return {
        isValid: false,
  ...
  public async processSingleFile(file: MulterFile, _userId?: string): Promise<FileUploadResult> {
    try {
      ...
      logger.info(
        `File uploaded successfully: ${file.originalname} (${(file.size / 1024).toFixed(2)}KB)`
      );

      return result;
    } catch (error) {
      logger.error(
        `Error processing uploaded file: ${error instanceof Error ? error.message : String(error)}`
      );
  ...
  public async processMultipleFiles(
    ...
    logger.info(
      `Batch upload completed: ${result.successCount}/${result.totalFiles} files uploaded successfully`
    );
  ...
  public async deleteFile(fileId: string): Promise<boolean> {
    try {
      ...
      logger.error(
        `Error deleting file ${fileId}: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
