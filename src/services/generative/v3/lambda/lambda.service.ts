import { LambdaClient, InvokeCommand, InvocationResponse, InvokeCommandInput } from '@aws-sdk/client-lambda';
...
export class LambdaService {
     ...
     */
    public async triggerAsync(functionName: string, payload: Record<string, any>): Promise<LambdaTriggerResult> {
        ...
        try {
            ...
            if (parsedResponse.body && typeof parsedResponse.body === 'string') {
                try {
                    ...
                    logger.debug('Lambda response body is not valid JSON, keeping as string', innerError);
                }
            }
    ...
    public async trigger(options: LambdaTriggerOptions): Promise<LambdaTriggerResult> {
        ...
        try {
            ...
            if (response.FunctionError) {
                logger.error(`Lambda function ${functionName} returned an error: ${response.FunctionError}`);
                return {
                    success: false,
