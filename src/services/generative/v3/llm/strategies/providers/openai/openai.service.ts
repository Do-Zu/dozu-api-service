...
export class OpenAIService extends BaseLLMProvider {
    ...
    public override async generate(prompt: string, options?: GenerationOptions): Promise<string | undefined | null> {
    ...
    private async createStream(
        ...
        try {
            ...
            logger.error(`Error creating OpenAI stream: ${error instanceof Error ? error.message : String(error)}`);
            return undefined;
        }
    }
    /**
    ...
    private async createCompletion(
        ...
        try {
            ...
            logger.error(`Error creating OpenAI completion: ${error instanceof Error ? error.message : String(error)}`);
            return undefined;
        }
    }
