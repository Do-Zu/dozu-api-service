...
function validateData<T extends ZodType>({ selector, schema }: { selector: (req: Request) => any, schema: T }) {
    return (req: Request, res: Response, next: NextFunction) => {
        try {
            const reqField = selector(req);
            schema.parse(reqField);
            next();
        } catch(err) {
...
export const validateParams = () => validateData<z.ZodObject<any, any>>({ selector: (req) => req.params, schema: idSchema });
