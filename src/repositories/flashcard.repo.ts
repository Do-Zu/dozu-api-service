...
import { IFlashcardAdded, IFlashcardDeleted, IFlashcardFieldsReturned, IFlashcardPracticed, IFlashcardProgressUpdated, IFlashcardReturned, IFlashcardUpdated } from '@/types/flashcard/flashcard.type';
...
const handleGetSingleFlashcard = async(flashcardId: number, fieldsReturned: IFlashcardFieldsReturned) => {
...
const handleGetFlashcardsPracticedForUser = async(userId: number, currentDate: Date) : Promise<IFlashcardPracticed[]> => {
    const flashcards = await db
        .select({
    ...
    const flashcardsFormatted = flashcards.map((flashcard) => {
        return {...flashcard, status: flashcard.status!, reviewInterval: flashcard.reviewInterval!, easinessFactor: flashcard.easinessFactor!, repetitionNumber: flashcard.repetitionNumber!}
    })
...
const handleUpdateFlashcardProgress = async(flashcardId: number, infoUpdated: IFlashcardProgressUpdated, fieldsReturned: IFlashcardFieldsReturned) => {
